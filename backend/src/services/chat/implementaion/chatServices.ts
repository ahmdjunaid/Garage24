...
export class ChatService implements IChatService {
  ...
  async getAppointmentsForChat(
    ...
    if (mechanicId) {
      const mechanic =
        await this._mechanicRepository.findOneByUserId(mechanicId);
      if (!mechanic) throw new AppError(HttpStatus.BAD_REQUEST, INVALID_INPUT);
      return await this._appointmentRepository.getAppointmentsForChat({
  ...
  async getAppointmentsForChatById(
    ...
    return await this._appointmentRepository.getAppointmentsForChatById(
      appointmentId
    );
  }
  ...
  async getUnreadCount(
    userId: string,
    role: string
  ): Promise<{ appointments: Record<string, number>; total: number }> {
    ...
    if (role === "mechanic") {
      ...
      if (!mechanic) throw new AppError(HttpStatus.BAD_REQUEST, INVALID_INPUT);
      currentUID = mechanic?.userId?.toString();
    }

    const appointmentIds =
      await this._appointmentRepository.getAppointmentsIdsForChat(currentUID);

    const unreadCounts = await this._chatRepository.getUnreadCount(
      appointmentIds,
      currentUID
    );
  ...
  async markAsRead(appointmentId: string, userId: string): Promise<UpdateResult> {
      return await this._chatRepository.markAsRead(appointmentId, userId)
  }
}
