...
import { fetchApptsByMechIdForChatApi } from "../services/chatServices";
...
export default function ChatMechanic() {
  ...
  useEffect(() => {
    ...
    const fetchAppointments = async (mechanicId: string) => {
      try {
        const response = await fetchApptsByMechIdForChatApi(mechanicId);
